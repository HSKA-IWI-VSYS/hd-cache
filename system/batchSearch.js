...
// A wrapper to expose the lambda as object, so that it can be called as in AWS Lambda.
return (function() {
	...
	//   --------------------------------	
	//
	//	Global attributes for the lambda container. Persists after lambda execution,
	...
	// Decides if to use AWS CloudWatch (false) or console.log (true)
	...
	function B_g_Q_(a, L_TRENCH, g)	{
		...
		return new Promise(async function(resolve, reject) {
			...
			// Gathers the data from a mock foreign API served over a MariaDB database.
				...
				const field = L_TRENCH ? UNIQUE_IDENTIFICATOR : a.v;
				let hdQuery = 'SELECT * FROM hd_mock_' + MOCK_NAME_HD + ' WHERE ';
				if(L_TRENCH)	{
...
//	Main methods of our proposed solution.
	...
	exports.handler = async function(event, context, callback) {
		...
		if(event.parameter.dig)	{
			...
			for(var i=0; !exit && i < event.parameter.dig.length; i++)	{	
				...
				const mocksave = MOCK_NAME;
				MOCK_NAME = event.parameter.dry ? 'dry_' + MOCK_NAME : MOCK_NAME;
				// executes RANK-SHRINK, the previous sate-of-the-art-solution.
		...
		if(typeof callback === 'function')	{
			//context.scucess(runCompleted === null ? FOREIGN_API_CALLS : false);
			callback(null, runCompleted === null ? FOREIGN_API_CALLS : false);
		}
		else {
	...
	// Can be uncommented to immediately start the lambda on initialization.
	/*exports.handler({
		parameter: {
			dig: [/*{
				v: 'sn',
